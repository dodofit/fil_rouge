"""
Classes and functions to implement the LSTM model for electricity price forecasting. The module does not
...
from tensorflow.keras.layers import Dense, Input, Dropout, AlphaDropout, BatchNormalization, LSTM
...
class LSTMModel(object):
    ...
    """Basic LSTM model based on keras and tensorflow. 
    
    The model can be used standalone to train and predict a LSTM using its fit/predict methods.
    However, it is intended to be used within the :class:`hyperparameter_optimizer` method
    and the :class:`LSTM` class. The former obtains a set of best hyperparameter using the :class:`LSTMModel` class. 
    The latter employes the set of best hyperparameters to recalibrate a :class:`LSTMModel` object
    ...
    neurons : list
        ...
        the LSTM model has an input layer of size ``n_features``, two hidden layers, and an output 
    ...
    optimizer : str, optional
        Name of the optimizer when training the LSTM. See the `keras documentation <https://keras.io/>`_ 
    ...
    def _reg(self, lambda_reg):
        """Internal method to build an l1 or l2 regularizer for the LSTM
    ...
    def _build_model(self):
        """Internal method that defines the structure of the LSTM
        ...
        """
        inputShape = (None, None, self.n_features)
        ...
        for k, neurons in enumerate(self.neurons):
            ...
            if self.activation == 'LeakyReLU':
               past_LSTM = LSTM(neurons,  input_shape=inputShape,activation='tanh',
                                   kernel_initializer=self.initializer,
                                   kernel_regularizer=self._reg(self.lambda_reg),
                                   return_sequences=True)(past_LSTM)
               past_LSTM = LSTM(neurons, activation='tanh', bbatch_atch_input_shape=inputShape,
                                   kernel_initializer=self.initializer,
                                   kernel_regularizer=self._reg(self.lambda_reg))(past_LSTM)
 
               past_LSTM = LeakyReLU(alpha=.001)(past_LSTM)
                ...
                past_LSTM = LSTM(neurons,  input_shape=inputShape,activation='tanh',
                                   ...
                                   kernel_regularizer=self._reg(self.lambda_reg),
                                   return_sequences=True)(past_LSTM)
                past_LSTM = LSTM(neurons, activation='tanh', input_shape=inputShape,
                                   kernel_initializer=self.initializer,
                                   kernel_regularizer=self._reg(self.lambda_reg))(past_LSTM)
 
                past_LSTM = PReLU()(past_LSTM)

            else:
                past_LSTM = LSTM(neurons, input_shape=inputShape, activation = 'tanh',
                                   kernel_initializer=self.initializer,
                                   kernel_regularizer=self._reg(self.lambda_reg),
                                   return_sequences=True)(past_data)
                print(past_LSTM)
                past_LSTM = LSTM(neurons,
                                   kernel_initializer=self.initializer, activation = 'tanh',
                                   kernel_regularizer=self._reg(self.lambda_reg))(past_LSTM)
  

        if self.batch_normalization:
                past_LSTM = BatchNormalization()(past_LSTM)

        if self.dropout > 0:
            if self.activation == 'selu':
                past_LSTM = AlphaDropout(self.dropout)(past_LSTM)
            else:
                past_LSTM = Dropout(self.dropout)(past_LSTM)
    ...
    def _obtain_metrics(self, X, Y):
        ...
        list
            ...
            representing the MAE of the LSTM
        """
    ...
    def fit(self, trainX, trainY, valX, valY):
        """Method to estimate the LSTM model.
    ...
    def predict(self, X):
        """Method to make a prediction after the LSTM is trained.
        
        Parameters
        ----------
        X : numpy.array
            Input to the LSTM. It has to be of size *[n, n_features]* where *n* can be any 
            integer, and *n_features* is the attribute of the LSTM representing the number of
            input features.
        
        Returns
        -------
        numpy.array
            Output of the LSTM after making the prediction.
        """
    ...
    def clear_session(self):
        ...
        It is used in the :class:`LSTM` class during recalibration to avoid RAM memory leakages.
        In particular, if the LSTM is retrained continuosly, at each step tensorflow slightly increases 
        the total RAM usage.
...
class my_LSTM(object):
    ...
    """LSTM for electricity price forecasting. 
    
    It considers a set of best hyperparameters, it recalibrates a :class:`LSTMModel` based on
    these hyperparameters, and makes new predictions.
    
    The difference w.r.t. the :class:`LSTMModel` class lies on the functionality. The
    :class:`LSTMModel` class provides a simple interface to build a keras LSTM model which
    ...
    nlayers : int, optional
        Number of layers of the LSTM model
    ...
    def _read_best_hyperapameters(self):
        ...
        trials_file_name = \
            'LSTM_hyperparameters_nl' + str(self.nlayers) + \
    ...
    def _regularize_data(self, Xtrain, Xval, Xtest, Ytrain, Yval):
        """Internal method to scale the input/outputs of the LSTM.
    ...
    def recalibrate(self, Xtrain, Ytrain, Xval, Yval):
        ...
        The method receives the training and validation dataset, and trains a :class:`LSTMModel` model
        ...
        self.model = LSTMModel(neurons=neurons, n_features=Xtrain.shape[-1], 
    ...
    def recalibrate_predict(self, Xtrain, Ytrain, Xval, Yval, Xtest):
        """Method that first recalibrates the LSTM model and then makes a prediction.
    ...
    def predict(self, X):
        ...
        # Predicting the current date using a recalibrated LSTM
    ...
    def recalibrate_and_forecast_next_day(self, df, next_day_date):
        """Method that builds an easy-to-use interface for daily recalibration and forecasting of the LSTM model
...
def evaluate_lstm_in_test_dataset(experiment_id, path_datasets_folder=os.path.join('.', 'datasets'), 
    ...
    """Function for easy evaluation of the LSTM model in a test dataset using daily recalibration. 
    
    The test dataset is defined by a market name and the test dates dates. The function
    generates the test and training datasets, and evaluates a LSTM model considering daily recalibration
    ...
    forecast_file_name = 'LSTM_forecast_nl' + str(nlayers) + '_dat' + str(dataset) + \
    ...
    model = my_LSTM(experiment_id=experiment_id, path_hyperparameter_folder=path_hyperparameter_folder,
...
def _build_and_split_XYs(dfTrain, features, shuffle_train, n_exogenous_inputs, dfTest=None, percentage_val=0.25,
    ...
    """Method to buil the X,Y pairs for training/test LSTM models using dataframes and a list of
    the selected inputs
