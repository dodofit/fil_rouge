...
def read_data_refreshed(dataset='BE',timezone = 'Europe/Brussels', years_test=2, start_date_dataset='2018-12-31',end_date_dataset=None,begin_test_date=None, end_test_date=None, api_key = '4e524938-86d0-4232-81a9-4c2aaf8cdceb'):
    ...
    start_date_dataset = pd.to_datetime(start_date_dataset)
    end_date_dataset = pd.to_datetime(date.today() + timedelta(days=4))

    df_price = client.query_day_ahead_prices(country_code = dataset, start=start_date_dataset.tz_localize(timezone), end=end_date_dataset.tz_localize(timezone), resolution='60T')
    df_load = client.query_load_forecast(country_code=dataset, start=start_date_dataset.tz_localize(timezone), end=end_date_dataset.tz_localize(timezone))
    df_generation = client.query_generation_forecast(country_code=dataset, start=start_date_dataset.tz_localize(timezone), end=end_date_dataset.tz_localize(timezone))

    df_price = pd.DataFrame(df_price, columns=['Price'])
    df_all = pd.merge_asof(df_price, pd.merge_asof(df_load, df_generation, left_index=True, right_index=True),
